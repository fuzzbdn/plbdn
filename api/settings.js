    ...
    try {
        if (req.method === 'GET') {
            ...
            if (type === 'display_bundle') {
                // Ingen edge-cache — varje arbetsplats måste alltid få sin egna data.
                // s-maxage kan ge fel arbetsplats om Vercel cachar utan att ta hänsyn till workplace-parametern.
                res.setHeader('Cache-Control', 'no-store, max-age=0');
            } else {
                res.setHeader('Cache-Control', 'no-store, max-age=0');
            }

            // Tillåt endast display_bundle att passera utan inloggning — men bara om token stämmer.
            // Om auth.isAuthorized är false för display_bundle betyder det att token INTE stämde
            // mot DISPLAY_SECRET, och auth.workplace är då aldrig satt från query-parametern.
            if (!auth.isAuthorized) {
                if (type === 'display_bundle') {
                    return res.status(401).json({ error: "Ogiltig display-nyckel." });
                }
